# -*- coding: utf-8 -*-
"""
Created on Mon Jul 12 14:25:32 2021
...
data.loc[data['COUNTRYNM'].isin(except_country),'Except_country']=1

data=data.fillna({'Except_country':0})

data.loc[((data['HS']==85)|(data['HS']==84)),'HS_85_84']=1
data=data.fillna({'HS_85_84':0})
data['Except_country']=data['Except_country'].astype('object')
data['HS_85_84']=data['HS_85_84'].astype('object')
...
mask2=((data['KR_TRADE_HSCD_COUNTRYCD']==0)&(data['KR_2017']!=0))|((data['KR_TRADE_HSCD_COUNTRYCD']!=0)&(data['KR_2017']==0))
data100=data[~mask2]

data100.COUNTRYNM.value_counts()

check=data100[['KR_2017','KR_TRADE_HSCD_COUNTRYCD']]
check=np.log1p(check)
check[['HSCD','HS']]=data100[['HSCD','HS']]
check['COUNTRYCD']=data100['COUNTRYCD']
check['pred']=pred
check.info()

check['COUNTRYNM']=data100['COUNTRYNM']
...
y=model_c['y']
x=model_c.drop('y',axis=1)
x2=x[['TRADE_COUNTRYCD','KR_2017','SNDIST','TRADE_HSCD_COUNTRYCD','IC_BUS_EASE_DFRN_DB','KR_HSCD_COUNTRY_RATIO','TRADE_HSCD_RATIO','SP_POP_TOTL','RDIST','TARIFF_AVG','PA_NUS_FCRF','KR_RATIO','GDP_RATIO','KMDIST','TRADE_RATIO','KR_HSCD_RATIO','TRADE_HSCD','KR_COUNTRY_RATIO','NY_GDP_MKTP_CD']]
x2.info()
...
d10.iloc[:,:22]
mask2=np.expm1(d10['KR_HSCD_COUNTRY_RATIO'])>1

mask1=((d10['y']==0)&(d10['KR_2017']!=0))|((d10['y']!=0)&(d10['KR_2017']==0))
d101=d10[~mask1]
...
data1000=data1000[~mask2]


data1000.COUNTRYNM.value_counts()

data1000=data1000.reset_index().drop('index',axis=1)

check=data1000[['KR_2017','KR_TRADE_HSCD_COUNTRYCD']]
check=np.log1p(check)
check[['HSCD','HS']]=data1000[['HSCD','HS']]
check['COUNTRYCD']=data1000['COUNTRYCD']
check['pred']=pred_remove
check.info()

check['COUNTRYNM']=data1000['COUNTRYNM']
...
res.to_csv("./res_remove.csv")

outlier=data[data["KR_HSCD_COUNTRY_RATIO"]>1]
outlier.COUNTRYNM.value_counts()
...
y=model_c['y']
x=model_c.drop('y',axis=1)
x2=x[['TRADE_COUNTRYCD','SNDIST','TRADE_HSCD_COUNTRYCD','IC_BUS_EASE_DFRN_DB','KR_HSCD_COUNTRY_RATIO','TRADE_HSCD_RATIO','SP_POP_TOTL','RDIST','TARIFF_AVG','PA_NUS_FCRF','GDP_RATIO','KMDIST','TRADE_RATIO','KR_HSCD_RATIO','TRADE_HSCD','KR_COUNTRY_RATIO','NY_GDP_MKTP_CD']]
x2.info()

name=x2.columns
name2=['COUNTRYNM_Netherlands','COUNTRYNM_Sri Lanka','IncomeGroup_Upper middle income']
...
x_num=ss.transform(x2)
x=pd.DataFrame(x_num)
x=pd.concat([x,model_c[['COUNTRYNM_Netherlands','COUNTRYNM_Sri Lanka','IncomeGroup_Upper middle income']]],axis=1)
x.columns=list(name)+name2
x.info()
...
for i in ['TRADE_COUNTRYCD','SNDIST','KR_RATIO','TRADE_HSCD_COUNTRYCD','IC_BUS_EASE_DFRN_DB','KR_HSCD_COUNTRY_RATIO','TRADE_HSCD_RATIO','RDIST','TARIFF_AVG','PA_NUS_FCRF','GDP_RATIO','KMDIST','KR_HSCD_RATIO','TRADE_HSCD','KR_COUNTRY_RATIO','NY_GDP_MKTP_CD']:
    model_c=d100
    model_c=model_c.reset_index().drop('index',axis=1)
    y=model_c['y']
    x=model_c.drop('y',axis=1)
    ...
    model_compare=d10[['TRADE_COUNTRYCD','SNDIST','KR_RATIO','TRADE_HSCD_COUNTRYCD','IC_BUS_EASE_DFRN_DB','KR_HSCD_COUNTRY_RATIO','TRADE_HSCD_RATIO','RDIST','TARIFF_AVG','PA_NUS_FCRF','GDP_RATIO','KMDIST','KR_HSCD_RATIO','TRADE_HSCD','KR_COUNTRY_RATIO','NY_GDP_MKTP_CD','y']]
    y_c=model_compare['y']
    x_c=model_compare.drop('y',axis=1)
    x2_c=x_c[['TRADE_COUNTRYCD','SNDIST','KR_RATIO','TRADE_HSCD_COUNTRYCD','IC_BUS_EASE_DFRN_DB','KR_HSCD_COUNTRY_RATIO','TRADE_HSCD_RATIO','RDIST','TARIFF_AVG','PA_NUS_FCRF','GDP_RATIO','KMDIST','KR_HSCD_RATIO','TRADE_HSCD','KR_COUNTRY_RATIO','NY_GDP_MKTP_CD']]
